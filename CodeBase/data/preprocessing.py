...
def augmentData(data, image_path='data/SDD/train', images={}, imageFile='reference.jpg', segMask=False):
	'''
	...
	for scene in data.sceneId.unique():
		im_path = os.path.join(image_path, scene, imageFile)
		if segMask:
...
def createImagesDict(data, imagePath, imageFile='reference.jpg'):
	images = {}
	for scene in data.sceneId.unique():
		if imageFile == 'oracle.png':
			im = cv2.imread(os.path.join(imagePath, scene, imageFile), 0)
		else:
			im = cv2.imread(os.path.join(imagePath, scene, imageFile))
		images[scene] = im
	return images
